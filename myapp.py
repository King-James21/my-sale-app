...
conn = st.connection("gsheets", type=GSheetsConnection, spreadsheet="https://docs.google.com/spreadsheets/d/1-yvBDHWkA__r3lsY-yGEcxb1kWz608yzZkIVQuzch4o/edit#gid=0")
...
if st.button("Submit sale", key="manual_entry"):
    if item and location:
        ...
        new_row = pd.DataFrame([{ "Date": str(sale_date), "Item": item,  "Location": location, "Quantity": quantity,"Price": price, "Total": price * quantity}])
...
if not df_history.empty:
  ...
  with col2:
    ...
    st.metric("Total Item Sold",int(total_qty))
    ...
    st.info("No sales history found yet.")
